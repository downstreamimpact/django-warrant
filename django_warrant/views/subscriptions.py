...
class AdminSubscriptions(UserPassesTestMixin, GetCognitoUserMixin, FormView):
    template_name = "warrant/admin-subscriptions.html"
    form_class = APIKeySubscriptionForm

    def get_success_url(self):
        return reverse_lazy("dw:admin-cognito-user", args=[self.kwargs.get("username")])
    ...
    def get_user_object(self):
        ...
        ).admin_get_user(attr_map=settings.COGNITO_ATTR_MAPPING)
    ...
    def get_form_kwargs(self):
        ...
        kwargs.update(
            {
                "plans": self.client.get_usage_plans().get("items", []),
